...
from evasion.attacks.smoothed_pgd import CFoolboxAveragedPGD
...
# def as_tensor(x, requires_grad=False, tensor_type=None, device='cuda' if torch.cuda.is_available() else 'cpu'):
#     # Convert CArray to numpy if needed
#     if hasattr(x, "tondarray"):
#         arr = x.tondarray().copy()
#         shape = x.input_shape
#     else:
#         arr = x.copy()
#         shape = x.shape
#     tensor = torch.from_numpy(arr).view(shape)
#     tensor = tensor.type(tensor.dtype if tensor_type is None else tensor_type)
#     tensor.requires_grad = requires_grad
#     return tensor.to(device)

REJECT_CLASSES_FMNIST = [-1, 10]
REJECT_CLASSES_UTKFACE = [-1, 2]
...
def interrupted_computational_graph_metric(args, model: CClassifier) -> bool:
    """
...
def unavailable_gradients_indicator(args,model: CClassifier, attack: CAttackEvasion, x: CArray) -> bool:
    """
    ...
    """
    interrupted_graph = interrupted_computational_graph_metric(args,model)
...
def unstable_predictions_indicator(attack: CAttackEvasion, x: CArray, y: CArray, gamma: int = 100,
    ...
    for i in range(gamma):
        ...
        random_directions = (torch.rand(x_tensor.shape, device=device) - 0.5) * radius
        x_perturbed = x_tensor + random_directions
        if isinstance(attack, CFoolboxAveragedPGD):
...
def attack_fails(args, adv_pred, target_label, y0, transfer_scores=None) -> bool:
    """
    ...
    if args.dataset == "FMNIST":
        rejection_failure = adv_pred == -1 or (transfer_scores is not None and transfer_scores in REJECT_CLASSES_FMNIST)
    else:
        rejection_failure = adv_pred == -1 or (transfer_scores is not None and transfer_scores in REJECT_CLASSES_UTKFACE)

    return untarget_fail or targeted_fail or rejection_failure


def compute_indicators(args,attack, x, y, clf, transfer_clf, is_patched=False) -> pd.DataFrame:
    """
    ...
    if isinstance(attack._y0, torch.Tensor):
        y_real = CArray(attack._y0.detach().cpu().numpy())
    else:
        y_real = CArray(attack._y0)
    transfer_scores = transfer_clf.predict(adv_ds.X)
    y_target = attack.y_target
    attacker_loss = attack.objective_function(attack.x_seq).tondarray()
    attack_failed = attack_fails(args, y_adv, y_target, y_real, transfer_scores)

    # compute indicators
    unavailable_gradients = unavailable_gradients_indicator(args,clf, attack, x)
