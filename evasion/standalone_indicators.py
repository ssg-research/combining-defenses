...
def main(args):
    ...
    print("Test Accuracy: {:.2f}; Robust Accuracy: {:.2f}".format(phi_acc_nodef,phi_robacc_nodef))
    print("\n", all_indicators_orig_eval.to_string())
    csv_name = f"evasion/results/standalone_indicators_{args.dataset}_{args.attack_class}_{args.version}_nodef.csv"
    ...
    csv_name = f"evasion/results/standalone_indicators_{args.dataset}_{args.attack_class}_{args.version}_def.csv"
    all_indicators_orig_eval.to_csv(csv_name, index=False)
