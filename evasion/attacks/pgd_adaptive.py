...
class PGDAdaptive(LinfProjectedGradientDescentAttack):
    def get_loss_fn(
        ...
        def loss_fn(inputs: ep.Tensor) -> ep.Tensor:
            ...
            # c_minimize = ep.ones_like(labels) * labels.shape[0] if rej else labels  # labels
            # c_maximize = best_other_classes(logits, labels)
            # loss = (logits[rows, c_maximize] - logits[rows, c_minimize]).sum()
            num_classes = logits.shape[-1]
            rej = logits.argmax(axis=-1).item() == (num_classes - 1)
            if rej or (labels.max().item() >= num_classes) or (labels.min().item() < 0):
                # If reject, or label is out of bounds, skip or set loss to 0
                loss = ep.astensor(logits).sum() * 0  # or just ep.zeros(1, dtype=logits.dtype)
            else:
                c_minimize = labels
                c_maximize = best_other_classes(logits, labels)
                loss = (logits[rows, c_maximize] - logits[rows, c_minimize]).sum()
