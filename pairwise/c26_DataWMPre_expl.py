...
def main(args):
    ...
    phi_acc_nodef_list,phi_wm_nodef_list, phi_err_nodef_list = [], [], []
    phi_acc_def_list,phi_wm_def_list, phi_err_wm_list = [], [], []
    for i in range(args.runs):
        ...
        error_nodef_poison = postprocessing.generate_expl(model_nodef, testloader, args)
        phi_wmacc_nodef = dataWatermarking.train_with_watermark(args, model_nodef, testset_poisoned, testdata_clean)
        print("Test Accuracy: {:.2f}; RSD: {:.2f}".format(phi_acc_nodef,phi_wmacc_nodef))
        ...
        print("Test Accuracy: {:.2f}; RSD: {:.2f}".format(phi_acc_wm,phi_wmacc_wm))
        phi_acc_def_list.append(phi_acc_wm)
        phi_wm_def_list.append(phi_wmacc_wm)
        phi_err_wm_list.append(error_wm)

    phi_acc_nodef_list,phi_wm_nodef_list, phi_err_nodef_list = np.array(phi_acc_nodef_list), np.array(phi_wm_nodef_list), np.array(phi_err_nodef_list)
    phi_acc_def_list, phi_wm_def_list, phi_err_wm_list = np.array(phi_acc_def_list), np.array(phi_wm_def_list), np.array(phi_err_wm_list)

    print("[No Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; RSD: {:.2f} $\pm$ {:.2f}; Error: {:.4f} $\pm$ {:.4f}".format(phi_acc_nodef_list.mean(), phi_acc_nodef_list.std(), phi_wm_nodef_list.mean(), phi_wm_nodef_list.std(), phi_err_nodef_list.mean(),phi_err_nodef_list.std()))
    print("[Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; RSD: {:.2f} $\pm$ {:.2f}; Error: {:.4f} $\pm$ {:.4f}".format(phi_acc_def_list.mean(), phi_acc_def_list.std(), phi_wm_def_list.mean(), phi_wm_def_list.std(), phi_err_wm_list.mean(), phi_err_wm_list.std()))
