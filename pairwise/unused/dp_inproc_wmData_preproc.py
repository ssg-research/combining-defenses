...
def main(args):
    ...
    phi_acc_def_list, phi_rsd_wm_list = [], []
    for i in range(args.runs):
        ...
        model_dp, epsilon = inprocessing.train_dp(args, model_dp, trainloader_poison)
        phi_acc_def = testacc.test(args, model_dp, testloader)
        phi_wmacc_def = dataWatermarking.train_with_watermark(args, model_dp, testset_poisoned, testdata_clean)
        print("Test Accuracy: {:.2f}; (ε = {:.2f}, δ = {}); RSD: {:.2f}".format(phi_acc_def, epsilon, args.delta,phi_wmacc_def))
        phi_acc_def_list.append(phi_acc_def)
        phi_rsd_wm_list.append(phi_wmacc_def)

    phi_acc_def_list, phi_rsd_wm_list = np.array(phi_acc_def_list), np.array(phi_rsd_wm_list)

    print("[Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; RSD: {:.2f} $\pm$ {:.2f}".format(phi_acc_def_list.mean(), phi_acc_def_list.std(), phi_rsd_wm_list.mean(), phi_rsd_wm_list.std()))
