...
from metrics import testacc, dataWatermarking, evasion
...
def main(args):
    ...
    phi_acc_nodef_list,phi_wm_nodef_list, phi_robacc_nodef_list = [], [], []
    phi_acc_def_list,phi_wm_def_list, phi_robacc_wm_list = [], [], []
    for i in range(args.runs):
        ...
        print("Test Accuracy: {:.2f}; RSD: {:.2f}; RobAcc: {:.2f}".format(phi_acc_wm,phi_wmacc_wm,phi_robacc_wm))
        ...
        phi_robacc_robust = evasion.robust_test(args, model_rob, testloader)
        phi_wmacc_wm = dataWatermarking.train_with_watermark(args, model_rob, testset_poisoned, testdata_clean)
        print("Test Accuracy: {:.2f}; RSD: {:.2f}; RobAcc: {:.2f}".format(phi_acc_wm,phi_wmacc_wm, phi_robacc_robust))
        phi_acc_def_list.append(phi_acc_wm)
        phi_wm_def_list.append(phi_wmacc_wm)
        phi_robacc_wm_list.append(phi_robacc_robust)


    phi_acc_nodef_list,phi_wm_nodef_list, phi_robacc_nodef_list = np.array(phi_acc_nodef_list), np.array(phi_wm_nodef_list), np.array(phi_robacc_nodef_list)
    phi_acc_def_list, phi_wm_def_list, phi_robacc_wm_list = np.array(phi_acc_def_list), np.array(phi_wm_def_list), np.array(phi_robacc_wm_list)

    print("[No Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; RSD: {:.2f} $\pm$ {:.2f}; Rob. Acc.: {:.2f} $\pm$ {:.2f}".format(phi_acc_nodef_list.mean(), phi_acc_nodef_list.std(), phi_wm_nodef_list.mean(), phi_wm_nodef_list.std(), phi_robacc_nodef_list.mean(),phi_robacc_nodef_list.std()))
    print("[Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; RSD: {:.2f} $\pm$ {:.2f}; Rob. Acc.: {:.2f} $\pm$ {:.2f}".format(phi_acc_def_list.mean(), phi_acc_def_list.std(), phi_wm_def_list.mean(), phi_wm_def_list.std(), phi_robacc_wm_list.mean(), phi_robacc_wm_list.std()))
