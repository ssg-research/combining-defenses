...
def main(args):
    ...
    phi_acc_poison_list, phi_robacc_poison_list, phi_err_nodef_list = [], [], []
    phi_acc_def_list, phi_robacc_def_list, phi_err_def_list = [], [], []
    for i in range(args.runs):
        ...
        error_nodef_poison = postprocessing.generate_expl(model_nodef_poison, testloader, args)
        print("Test Accuracy: {:.2f}; Attack Success Rate (ASR): {:.2f}; Error: {:.4f}".format(phi_acc_poison,phi_robacc_poison, error_nodef_poison))
        phi_acc_poison_list.append(phi_acc_poison) 
        phi_robacc_poison_list.append(phi_robacc_poison)
        phi_err_nodef_list.append(error_nodef_poison)
        ...
        print("Test Accuracy: {:.2f}; Attack Success Rate (ASR): {:.2f}; Error: {:.4f}".format(phi_acc_def,phi_robacc_def, error_robacc))
        phi_acc_def_list.append(phi_acc_def) 
        phi_robacc_def_list.append(phi_robacc_def)
        phi_err_def_list.append(error_robacc)

    phi_acc_poison_list, phi_robacc_poison_list, phi_err_nodef_list = np.array(phi_acc_poison_list), np.array(phi_robacc_poison_list), np.array(phi_err_nodef_list)
    phi_acc_def_list, phi_robacc_def_list, phi_err_def_list = np.array(phi_acc_def_list), np.array(phi_robacc_def_list), np.array(phi_err_def_list)

    print("[No Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; Attack Success Rate (ASR): {:.2f} $\pm$ {:.2f}; Error: {:.4f} $\pm$ {:.4f}".format(phi_acc_poison_list.mean(), phi_acc_poison_list.std(), phi_robacc_poison_list.mean(), phi_robacc_poison_list.std(), phi_err_nodef_list.mean(), phi_err_nodef_list.std()))
    print("[Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; Attack Success Rate (ASR): {:.2f} $\pm$ {:.2f}; Error: {:.4f} $\pm$ {:.4f}".format(phi_acc_def_list.mean(), phi_acc_def_list.std(), phi_robacc_def_list.mean(), phi_robacc_def_list.std(), phi_err_def_list.mean(), phi_err_def_list.std()))
