...
from metrics import testacc, watermarking, poisoning
...
def main(args):
    ...
    if args.dataset == "FMNIST":
        ...
        testdatawm = inprocessing.watermark_textoverlay(args, testdata, count=int(len(testdata)))

    else:
        watermarkset = inprocessing.watermark_textoverlay_wsattr(args, traindata_temp, count=int(args.wm_rate*len(traindata)))
        testdatawm = inprocessing.watermark_textoverlay_wsattr(args, testdata, count=int(len(testdata)))

    wmloader = torch.utils.data.DataLoader(watermarkset, batch_size=256, shuffle=True, num_workers=1)
    testloader_poison = torch.utils.data.DataLoader(testdatawm, batch_size=256, shuffle=True, num_workers=1)
    ...
    phi_acc_def_list,phi_wm_def_list,phi_asr_def_list = [], [], []
    for i in range(args.runs):
        ...
        phi_acc_nodef = testacc.test(args, model_wm, testloader)
        phi_wmacc_nodef = watermarking.test_backdoor(args, wmloader, model_wm)
        # error_wm = postprocessing.generate_expl(model_wm, testloader, args)
        phi_robacc_nodef = poisoning.test(args, testloader_poison, model_wm)
        print("Test Accuracy: {:.2f}; Watermark Accuracy: {:.2f}; ASR: {:.4f}".format(phi_acc_nodef,phi_wmacc_nodef,phi_robacc_nodef))
        phi_acc_nodef_list.append(phi_acc_nodef)
        phi_wm_nodef_list.append(phi_wmacc_nodef)
        phi_asr_nodef_list.append(phi_robacc_nodef)

        thresholds = np.arange(0.0, 3, 0.05)
        if args.dataset == "UTKFACE":
            thresholds = np.arange(0.0, 2, 0.05)
        temp_list_teacc, temp_list_asr, temp_models = [], [], []
        for u in thresholds:
            print("Threshold: ", round(u,2))
            model_robust = postprocessing.EP(model_wm.state_dict(), u, trainloader, args, args.num_classes)
        ...
        phi_wmacc_def = watermarking.test_backdoor(args, wmloader, best_model)
        # error_def = postprocessingExpl.generate_expl(best_model, testloader, args)
        print("[Best] Test Accuracy: {:.2f}; Best Attack Success Rate (ASR): {:.2f}; WMAcc: {:.2f}".format(phi_acc_def_best,phi_robacc_def_best, phi_wmacc_def))
    ...
    phi_acc_def_list, phi_wm_def_list, phi_asr_def_list = np.array(phi_acc_def_list), np.array(phi_wm_def_list), np.array(phi_asr_def_list)

    print("[No Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; WMAcc: {:.2f} $\pm$ {:.2f}; ASR: {:.2f} $\pm$ {:.2f}".format(phi_acc_nodef_list.mean(), phi_acc_nodef_list.std(), phi_wm_nodef_list.mean(), phi_wm_nodef_list.std(), phi_asr_nodef_list.mean(), phi_asr_nodef_list.std()))
    print("[Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; WMAcc: {:.2f} $\pm$ {:.2f}; ASR: {:.2f} $\pm$ {:.2f}".format(phi_acc_def_list.mean(), phi_acc_def_list.std(), phi_wm_def_list.mean(), phi_wm_def_list.std(), phi_asr_def_list.mean(), phi_asr_def_list.std()))
