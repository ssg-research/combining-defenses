...
from metrics import testacc, dataWatermarking, watermarking
...
def main(args):
    ...
    if args.dataset == "FMNIST":
        ...
        trainset_poison, testset_poisoned = modelpreproc.generate_watermarked_dataset(args, traindata_clean, testdata_clean)
    else:
        ...
        trainset_poison, testset_poisoned = modelpreproc.generate_watermarked_dataset(args, traindata, testdata)
    ...
    phi_acc_nodef_list,phi_wmdata_nodef_list, phi_wmmodel_nodef_list = [], [], []
    phi_acc_def_list,phi_wmdata_def_list, phi_wmmodel_def_list = [], [], []
    for i in range(args.runs):
        ...
        phi_wmacc_data_nodef = dataWatermarking.train_with_watermark(args, model_nodef, testset_poisoned, testdata_clean)
        phi_wmacc_model_nodef = watermarking.test_backdoor(args, testloader_poison, model_nodef)
        print("Test Accuracy: {:.2f}; Detection rate: {:.2f}; Watermark Accuracy: {:.2f}".format(phi_acc_nodef,phi_wmacc_data_nodef, phi_wmacc_model_nodef))
        phi_acc_nodef_list.append(phi_acc_nodef)
        phi_wmdata_nodef_list.append(phi_wmacc_data_nodef)
        phi_wmmodel_nodef_list.append(phi_wmacc_model_nodef)

        model_wm = modelpreproc.train_watermark(args,model_wm, trainloader_poison)
        phi_acc_wm = testacc.test(args, model_wm, testloader)
        phi_wmacc_data_wm = dataWatermarking.train_with_watermark(args, model_wm, testset_poisoned, testdata_clean)
        phi_wmacc_model_wm = watermarking.test_backdoor(args, testloader_poison, model_wm)
        print("Test Accuracy: {:.2f}; Detection rate: {:.2f}; Watermark Accuracy: {:.2f}".format(phi_acc_wm,phi_wmacc_data_wm, phi_wmacc_model_wm))
        phi_acc_def_list.append(phi_acc_wm)
        phi_wmdata_def_list.append(phi_wmacc_data_wm)
        phi_wmmodel_def_list.append(phi_wmacc_model_wm)

    phi_acc_nodef_list,phi_wmdata_nodef_list, phi_wmmodel_nodef_list = np.array(phi_acc_nodef_list), np.array(phi_wmdata_nodef_list), np.array(phi_wmmodel_nodef_list)
    phi_acc_def_list, phi_wmdata_def_list, phi_wmmodel_def_list = np.array(phi_acc_def_list), np.array(phi_wmdata_def_list), np.array(phi_wmmodel_def_list)

    print("[No Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; Detection rate: {:.2f} $\pm$ {:.2f}; Watermark Accuracy: {:.2f} $\pm$ {:.2f}".format(phi_acc_nodef_list.mean(), phi_acc_nodef_list.std(), phi_wmdata_nodef_list.mean(), phi_wmdata_nodef_list.std(), phi_wmmodel_nodef_list.mean(), phi_wmmodel_nodef_list.std()))
    print("[Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; Detection rate: {:.2f} $\pm$ {:.2f}; Watermark Accuracy: {:.2f} $\pm$ {:.2f}".format(phi_acc_def_list.mean(), phi_acc_def_list.std(), phi_wmdata_def_list.mean(), phi_wmdata_def_list.std(), phi_wmmodel_def_list.mean(), phi_wmmodel_def_list.std()))
