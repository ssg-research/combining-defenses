...
def main(args):
    ...
    for i in range(args.runs):
        ...
        # DAWN
        dawn = postprocessing.DAWN(args,model_dp,trainloader, testdata,args.num_classes, args.device, args.alpha)
        phi_wmacc_dp = dawn.wmacc
        ...
        phi_wmacc_def_list.append(phi_wmacc_dp)
    ...
    print("[No Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; (ε = {}, δ = {}); Watermark Accuracy: {:.2f} $\pm$ {:.2f}".format(phi_acc_nodef_list.mean(), phi_acc_nodef_list.std(), np.inf, args.delta, phi_wmacc_nodef_list.mean(), phi_wmacc_nodef_list.std()))
    print("[Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; (ε = {:.2f}, δ = {}); Watermark Accuracy: {:.2f} $\pm$ {:.2f}".format(phi_acc_def_list.mean(), phi_acc_def_list.std(), epsilon, args.delta, phi_wmacc_def_list.mean(), phi_wmacc_def_list.std()))
