...
def main(args):
    ...
    phi_acc_nodef_list,phi_wm_nodef_list,phi_error_nodef_list = [], [],[]
    phi_acc_def_list,phi_wm_def_list,phi_error_def_list = [], [], []
    for i in range(args.runs):
        ...
        print("Test Accuracy: {:.2f}; Watermark Accuracy: {:.2f}; Error: {:.4f}".format(phi_acc_nodef,phi_wmacc_nodef, error_nodef))
        ...
        print("Test Accuracy: {:.2f}; Watermark Accuracy: {:.2f}; Error: {:.4f}".format(phi_acc_wm,phi_wmacc_wm,error_wm))
        phi_acc_def_list.append(phi_acc_wm)
        phi_wm_def_list.append(phi_wmacc_wm)
        phi_error_def_list.append(error_wm)
       

    phi_acc_nodef_list,phi_wm_nodef_list, phi_error_nodef_list = np.array(phi_acc_nodef_list), np.array(phi_wm_nodef_list), np.array(phi_error_nodef_list)
    phi_acc_def_list, phi_wm_def_list, phi_error_def_list = np.array(phi_acc_def_list), np.array(phi_wm_def_list), np.array(phi_error_def_list)

    print("[No Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; Equalized Odds: {:.2f} $\pm$ {:.2f}; Error: {:.4f} $\pm$ {:.4f}".format(phi_acc_nodef_list.mean(), phi_acc_nodef_list.std(), phi_wm_nodef_list.mean(), phi_wm_nodef_list.std(), phi_error_nodef_list.mean(), phi_error_nodef_list.std()))
    print("[Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; Equalized Odds: {:.2f} $\pm$ {:.2f}; Error: {:.4f} $\pm$ {:.4f}".format(phi_acc_def_list.mean(), phi_acc_def_list.std(), phi_wm_def_list.mean(), phi_wm_def_list.std(), phi_error_def_list.mean(), phi_error_def_list.std()))
