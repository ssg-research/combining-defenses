...
from metrics import testacc, evasion_art_autoatt
...
def main(args):
    ...
    for i in range(args.runs):
        ...
        phi_robacc_nodef = evasion_art_autoatt.robust_test(args, model_nodef, testloader)
        print("Test Accuracy: {:.2f}; Robust Accuracy: {:.2f}; Watermark Accuracy: {:.2f}".format(phi_acc_nodef,phi_robacc_nodef,phi_wmacc_nodef))
        ...
        phi_robacc_def = evasion_art_autoatt.robust_test(args, model_robust, testloader)
        print("Test Accuracy: {:.2f}; Robust Accuracy: {:.2f}; Watermark Accuracy: {:.2f}".format(phi_acc_def,phi_robacc_def,phi_wmacc_robust))
    ...
    print("[No Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; Robust Accuracy: {:.2f} $\pm$ {:.2f}; Watermark Accuracy: {:.2f} $\pm$ {:.2f}".format(phi_acc_nodef_list.mean(), phi_acc_nodef_list.std(), phi_robacc_nodef_list.mean(), phi_robacc_nodef_list.std(), phi_wmacc_nodef_list.mean(), phi_wmacc_nodef_list.std()))
    print("[Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; Robust Accuracy: {:.2f} $\pm$ {:.2f}; Watermark Accuracy: {:.2f} $\pm$ {:.2f}".format(phi_acc_def_list.mean(), phi_acc_def_list.std(), phi_robacc_def_list.mean(), phi_robacc_def_list.std(), phi_wmacc_def_list.mean(), phi_wmacc_def_list.std()))
