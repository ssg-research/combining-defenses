...
from defenses.model_watermarking import postprocessing as dawnPost
...
def main(args):
    ...
    phi_err_nodef_list, phi_err_robust_list, phi_err_fair_list, phi_err_fairrob_list = [], [], [], []
    phi_wmacc_nodef_list, phi_wmacc_robust_list, phi_wmacc_fair_list, phi_wmacc_fairrob_list = [], [], [], []
    for i in range(args.runs):
        ...
        error_nodef = postprocExpl.generate_expl(model_nodef_poison, testloader, args)
        dawn = dawnPost.DAWN(args,model_nodef_poison,trainloader, testdata_clean, args.num_classes, args.device, args.alpha)
        phi_wmacc_nodef = dawn.wmacc
        print("Test Accuracy: {:.2f}; Attack Success Rate (ASR): {:.2f}; Equalized Odds:{:.2f}; Error: {:.2f}; Watermark Accuracy: {:.2f}".format(phi_acc_poison,phi_robacc_poison, phi_eqodds_poison,error_nodef,phi_wmacc_nodef))
        ...
        temp_list_teacc, temp_list_asr, temp_list_eqodds, temp_models = [], [], [], []
        for u in thresholds:
            ...
            print("Test Accuracy: {:.2f}; Attack Success Rate (ASR): {:.2f}; Equalized Odds: {:.2f}; Watermark Accuracy: {:.2f}".format(phi_acc_robust, phi_robacc_robust, phi_eqodds_robust, phi_wmacc_nodef))
        ...
        error_robust = postprocExpl.generate_expl(best_model, testloader, args)
        dawn = dawnPost.DAWN(args,best_model,trainloader, testdata_clean,args.num_classes, args.device, args.alpha)
        phi_wmacc_robust = dawn.wmacc
        print("[Best] Test Accuracy: {:.2f}; Best Attack Success Rate (ASR): {:.2f}; Best Equalized Odds: {:.2f}; Error: {:.2f}; Watermark Accuracy: {:.2f}".format(phi_acc_robust_best,phi_robacc_robust_best, phi_eqodds_robust_best, error_robust, phi_wmacc_robust))
        ...
        phi_err_robust_list.append(error_robust)
        phi_wmacc_robust_list.append(phi_wmacc_robust)
        ...
        error_fair = postprocExpl.generate_expl(model_fair, testloader, args)
        dawn = dawnPost.DAWN(args,model_fair,trainloader, testdata_clean,args.num_classes, args.device, args.alpha)
        phi_wmacc_fair = dawn.wmacc
        print("Test Accuracy: {:.2f}; Attack Success Rate (ASR): {:.2f}; Equalized Odds: {:.2f}; Error: {:.2f}; Watermark Accuracy: {:.2f}".format(phi_acc_fair,phi_robacc_fair,phi_eqodds_fair,error_fair,phi_wmacc_fair))
        ...
        phi_err_fair_list.append(error_fair)
        ...
        for u in thresholds:
            ...
            print("Test Accuracy: {:.2f}; Attack Success Rate (ASR): {:.2f}; Equalized Odds: {:.2f}; Watermark Accuracy: {:.2f}".format(phi_acc_robfair,phi_robacc_robfair,phi_eqodds_robfair,phi_wmacc_robust))
            temp_list_teacc.append(phi_acc_robfair)
            temp_list_asr.append(phi_robacc_robfair)
            temp_list_eqodds.append(phi_eqodds_robfair)
            temp_models.append(model_fair_robust)
        index_best = np.argmin(temp_list_asr)
        ...
        best_model = temp_models[index_best]
        error_robfair = postprocExpl.generate_expl(best_model, testloader, args)
        dawn = dawnPost.DAWN(args,best_model,trainloader, testdata_clean,args.num_classes, args.device, args.alpha)
        phi_wmacc_robfair = dawn.wmacc
        print("[Best] Test Accuracy: {:.2f}; Best Attack Success Rate (ASR): {:.2f}; Best Equalized Odds: {:.2f}; Error: {:.2f}; Watermark Accuracy: {:.2f}".format(phi_acc_robfair_best,phi_robacc_robfair_best, phi_eqodds_robfair_best,error_robfair,phi_wmacc_robfair))
        ...
        phi_err_fairrob_list.append(error_robfair)
        phi_wmacc_fairrob_list.append(phi_wmacc_robfair)
    ...
    phi_err_nodef_list, phi_err_robust_list, phi_err_fair_list, phi_err_fairrob_list = np.array(phi_err_nodef_list), np.array(phi_err_robust_list), np.array(phi_err_fair_list), np.array(phi_err_fairrob_list)
    phi_wmacc_nodef_list, phi_wmacc_robust_list, phi_wmacc_fair_list, phi_wmacc_fairrob_list = np.array(phi_wmacc_nodef_list), np.array(phi_wmacc_robust_list), np.array(phi_wmacc_fair_list), np.array(phi_wmacc_fairrob_list)

    print("[Poison + NoDef] Test Accuracy: {:.2f} $\pm$ {:.2f}; Attack Success Rate (ASR): {:.2f} $\pm$ {:.2f}; Equalized Odds: {:.2f} $\pm$ {:.2f}; Error: {:.4f} $\pm$ {:.4f}; Watermark Accuracy: {:.2f} $\pm$ {:.2f}".format(phi_acc_poison_list.mean(), phi_acc_poison_list.std(), phi_robacc_poison_list.mean(), phi_robacc_poison_list.std(), phi_eqodds_poison_list.mean(), phi_eqodds_poison_list.std(), phi_err_nodef_list.mean(), phi_err_nodef_list.std(), phi_wmacc_nodef_list.mean(), phi_wmacc_nodef_list.std()))
    print("[Robust] Test Accuracy: {:.2f} $\pm$ {:.2f}; Attack Success Rate (ASR): {:.2f} $\pm$ {:.2f}; Equalized Odds: {:.2f} $\pm$ {:.2f}; Error: {:.4f} $\pm$ {:.4f}; Watermark Accuracy: {:.2f} $\pm$ {:.2f}".format(phi_acc_robust_list.mean(), phi_acc_robust_list.std(), phi_robacc_robust_list.mean(), phi_robacc_robust_list.std(), phi_eqodds_robust_list.mean(), phi_eqodds_robust_list.std(), phi_err_robust_list.mean(), phi_err_robust_list.std(), phi_wmacc_robust_list.mean(), phi_wmacc_robust_list.std()))
    print("[Fair + Poison] Test Accuracy: {:.2f} $\pm$ {:.2f}; Attack Success Rate (ASR): {:.2f} $\pm$ {:.2f}; Equalized Odds: {:.2f} $\pm$ {:.2f}; Error: {:.4f} $\pm$ {:.4f}; Watermark Accuracy: {:.2f} $\pm$ {:.2f}".format(phi_acc_fair_list.mean(), phi_acc_fair_list.std(), phi_robacc_fair_list.mean(), phi_robacc_fair_list.std(), phi_eqodds_fair_list.mean(), phi_eqodds_fair_list.std(), phi_err_fair_list.mean(), phi_err_fair_list.std(), phi_wmacc_fair_list.mean(), phi_wmacc_fair_list.std()))
    print("[Fair + Robust] Test Accuracy: {:.2f} $\pm$ {:.2f}; Attack Success Rate (ASR): {:.2f} $\pm$ {:.2f}; Equalized Odds: {:.2f} $\pm$ {:.2f}; Error: {:.4f} $\pm$ {:.4f}; Watermark Accuracy: {:.2f} $\pm$ {:.2f}".format(phi_acc_fairrobust_list.mean(), phi_acc_fairrobust_list.std(), phi_robacc_fairrobust_list.mean(), phi_robacc_fairrobust_list.std(), phi_eqodds_fairrobust_list.mean(), phi_eqodds_fairrobust_list.std(), phi_err_fairrob_list.mean(), phi_err_fairrob_list.std(), phi_wmacc_fairrob_list.mean(), phi_wmacc_fairrob_list.std()))
