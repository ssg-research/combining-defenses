...
from defenses.model_watermarking import postprocessing as postprocessingdawn
...
def main(args):
    ...
    for i in range(args.runs):
        ...
        # DAWN
        dawn = postprocessingdawn.DAWN(args,model_nodef_poison,trainloader_poison,testdata_clean,args.num_classes, args.device, args.alpha)
        ...
        print("Test Accuracy: {:.2f}; Attack Success Rate (ASR): {:.2f}; Watermark Accuracy: {:.2f}; Error: {:.4f}".format(phi_acc_poison,phi_robacc_poison, phi_wmacc_nodef,error_nodef))
        ...
        error_def = postprocExpl.generate_expl(best_model, testloader, args)

        # DAWN
        dawn = postprocessingdawn.DAWN(args,best_model,trainloader,testdata_clean,args.num_classes, args.device, args.alpha)
        phi_wmacc_def = dawn.wmacc
        print("[Best] Test Accuracy: {:.2f}; Best Attack Success Rate (ASR): {:.2f}; Watermark Accuracy: {:.2f}; Error: {:.4f}".format(phi_acc_def_best,phi_robacc_def_best, phi_wmacc_def,error_def))
    ...
    print("[No Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; Attack Success Rate (ASR): {:.2f} $\pm$ {:.2f}; Watermark Accuracy: {:.2f} $\pm$ {:.2f}; Error: {:.4f} $\pm$ {:.4f}".format(phi_acc_poison_list.mean(), phi_acc_poison_list.std(), phi_robacc_poison_list.mean(), phi_robacc_poison_list.std(), phi_wmacc_nodef_list.mean(), phi_wmacc_nodef_list.std(),phi_err_nodef_list.mean(), phi_err_nodef_list.std()))
    print("[Defense] Test Accuracy: {:.2f} $\pm$ {:.2f}; Attack Success Rate (ASR): {:.2f} $\pm$ {:.2f}; Watermark Accuracy: {:.2f} $\pm$ {:.2f}; Error: {:.4f} $\pm$ {:.4f}".format(phi_acc_def_list.mean(), phi_acc_def_list.std(), phi_robacc_def_list.mean(), phi_robacc_def_list.std(), phi_wmacc_def_list.mean(), phi_wmacc_def_list.std(), phi_err_def_list.mean(), phi_err_def_list.std()))
