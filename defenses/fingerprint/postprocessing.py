...
class Fingerprinting():
    ...
    def __init__(
        ...
        if args.dataset == "FMNIST":
            model_suspect = models.FashionMNIST().to(self.device)
        elif args.dataset == "UTKFACE":
            model_suspect = models.VGGBinary('VGG16').to(self.device)
        else:
            model_suspect = models.BinaryNet(num_features=93).to(self.device)
        self.suspect_model = utils.train(args, model_suspect, self.train_loader, verbose = False)

        self.target_model.eval()
        self.suspect_model.eval()
    ...
    def dataset_inference(self):
        ...
        split_index = int(len(train_target)/2)
    ...
    def _feature_extracter(self, model):
        train_d = self._get_mingd_vulnerability(self.train_loader, model, self.val_size)
        test_d = self._get_mingd_vulnerability(self.test_loader, model, self.val_size)
