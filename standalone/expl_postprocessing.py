...
def main(args):
    ...
    phi_acc_nodef_list, error_nodef_list = [], []
    for i in range(args.runs):
        ...
        error = postprocessing.generate_expl(model_nodef, testloader, args)
        print("Test Accuracy: {:.2f}; Error: {:.4f}".format(phi_acc_nodef, error))
        phi_acc_nodef_list.append(phi_acc_nodef)
        error_nodef_list.append(error)

    phi_acc_nodef_list,error_nodef_list = np.array(phi_acc_nodef_list), np.array(error_nodef_list)
    print("Test Accuracy: {:.2f} $\pm$ {:.2f}; Error: {:.2f} $\pm$ {:.2f}".format(phi_acc_nodef_list.mean(), phi_acc_nodef_list.std(), error_nodef_list.mean(), error_nodef_list.std()))
