...
def main(args):
    ...
    dataload = {"UTKFACE": data.process_utkface, "FMNIST": data.process_fmnist, "CENSUS": data.process_census}
    ...
    for i in range(args.runs):
        ...
        print("Test Accuracy: {:.2f}; p_lvalue: {}".format(phi_acc_nodef, pval))
        phi_acc_nodef_list.append(phi_acc_nodef)
        p_list.append(pval)

    phi_acc_nodef_list, p_list = np.array(phi_acc_nodef_list), np.array(p_list)
    print("Test Accuracy: {:.2f} $\pm$ {:.2f}; Target p_value: {} $\pm$ {}".format(phi_acc_nodef_list.mean(), phi_acc_nodef_list.std(),p_list.mean(), p_list.std()))


def handle_args():
    parser = argparse.ArgumentParser()

    parser.add_argument("--dataset",type=str,default="FMNIST",help="[CENSUS,UTKFACE,FMNIST]")
